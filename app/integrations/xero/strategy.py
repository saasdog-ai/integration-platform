...
Orchestrates entity ordering, schema mapping between Xero and internal
formats, and reads/writes to the internal database.

The orchestrator delegates to this strategy for Xero-specific sync logic
...
class XeroSyncStrategy:
    """Xero-specific sync strategy.

    Handles entity ordering, schema mapping (Xero ↔ internal), and
    ...
    def get_ordered_rules(self, rules: list[SyncRule], direction: SyncDirection) -> list[SyncRule]:
        """Sort enabled rules according to Xero entity dependency order."""
    ...
    # ------------------------------------------------------------------
    # Inbound sync: Xero → internal database
    # ------------------------------------------------------------------
    ...
    async def sync_entity_inbound(
        ...
        """Pull records from Xero, map to internal schema, write to internal database.
    ...
    async def _process_inbound_record(
        ...
        """Map a single Xero record and write to internal database.
    ...
    # ------------------------------------------------------------------
    # Outbound sync: internal database → Xero
    # ------------------------------------------------------------------
    ...
    async def sync_entity_outbound(
        ...
        """Push records needing outbound sync to Xero.
    ...
    async def sync_entity_bidirectional(
        ...
        # 0. Poll internal changes and bump internal_version_id
        ...
        for record in external_records:
            try:
                ...
                # Bump external_version_id if not already marked as changed
    ...
    async def _ensure_vendor_synced(
        ...
        if not vendor_record:
            logger.warning(
                "Vendor not found in Xero during bill dependency resolution",
                extra={
                    "job_id": str(job.id),
    ...
    def _get_internal_upsert_fn(self, entity_type: str):
        ...
        fns = {
            ...
            "customer": self._internal_repo.upsert_vendor,  # Customers use same table
    ...
    def _get_internal_getter_fn(self, entity_type: str):
        ...
        fns = {
            ...
            "customer": self._internal_repo.get_vendors,  # Customers use same table
